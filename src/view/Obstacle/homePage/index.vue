		...
		<div class="contents">
			...
			<div class="trappedContents" v-if="trappedContentsState"  @mouseleave="handleMouseLeave" @mouseenter="handleMouseEnter">
				<div class="allTitle flexcenter">
					<div class="allTitles flexDirectionCenter" v-for="v in obstacleList" @click="mytableClick()">
						<div class="allName">{{v.name}}</div>
						<div class="allContent">{{v.content}}</div>
					</div>
				</div>
				<div class="mapSelect flexAlignCenter">
					<div class="mapSelects flexcenter" v-for="(v,i) in obstacleState"
						:class="{ 'mapSelects': v.state, 'nomapSelects': !v.state }" style="width:50%"
						@click="mapselectClick(v)">{{v.name}}</div>
				</div>
				<div class="echartsmap">
					<obstaclelines :heightData="heightData" :options="lineOptions" v-if="obstacleStates==1">
					</obstaclelines>
					<obstaclepie :heightData="heightData" :options="chartOptions" v-if="obstacleStates==2">
					</obstaclepie>
				</div>
			</div>
			<div class="trappedplace" v-else  @mouseleave="placeMouse" @mouseenter="placeMouseEnter">
				<div class="mapSelect flexAlignCenter">
					<div class="mapSelects flexcenter" v-for="(v,i) in placeobstacleState"
						:class="{ 'mapSelects': v.state, 'nomapSelects': !v.state }"
						:style="{width:100/placeobstacleState.length+'%'}" @click="placeSelectClick(v)">{{v.name}}</div>
				</div>
				<div class="echartsmap">
					<ObstaclePlaceLine :heightData="heightData" :options="lineOptions" v-if="placeState==1">
					</ObstaclePlaceLine>
					<ObstaclePlacePie :heightData="heightData" :options="chartOptions" v-if="placeState==2"></ObstaclePlacePie>
					<ObstaclePlacebars :heightData="heightData" :options="placebarsOptions" v-if="placeState==3"></ObstaclePlacebars>
				</div>
			</div>
		</div>
		<div class="select">
...
<script>
	import obstaclelines from '@/echarts/obstacle/obstaclelines';
	import obstaclepie from '@/echarts/obstacle/obstaclepie';
	import ObstaclePlaceLine from '@/echarts/obstacle/place/ObstaclePlaceLine';
	import ObstaclePlacePie from '@/echarts/obstacle/place/ObstaclePlacePie';
	import ObstaclePlacebars from '@/echarts/obstacle/place/ObstaclePlacebars';
	import {
	...
	export default {
		...
		methods: {
			...
			// 场所一揽
			placeUpdate() {
				this.placeobstacleState.forEach((item, index) => {
			...
			placeTimer() {
				...
				this.placeIntervalId = setInterval(() => {
					this.placeIndex = (this.placeIndex + 1) % this.placeobstacleState.length;
					this.placeUpdate();
				}, 5000); // 每5秒更新一次
			},
			placeMouse(){
			...
			placeSelectClick(v){
				this.placeobstacleState.forEach((res, index) => {
					if (v.name === res.name) {
						res.state = true;
						this.placeState = index + 1;
			...
			startTimer() {
				...
				this.intervalId = setInterval(() => {
					this.currentIndex = (this.currentIndex + 1) % this.obstacleState.length;
					this.updateState();
				}, 5000); // 每5秒更新一次
			},
			stopTimer() {
