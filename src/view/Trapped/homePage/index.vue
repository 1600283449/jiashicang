		...
		<div class="contents">
			<div class="title">
				<div class="titles justifyBetween">
					<div class="titleLeft flexAlignCenter">
						...
						<div class="name">智慧救援（本月）</div>
					</div>
					<div class="TitleRight" @click="detailedClick()">
						详细统计 >
					</div>
				</div>
			</div>
			<div class="trappedContents" v-if="trappedContentsState"  @mouseleave="handleMouseLeave" @mouseenter="handleMouseEnter">
				<div class="allTitle flexcenter">
					<div class="allTitles flexDirectionCenter" v-for="v in TrappedList"  @click="mytableClick()">
						<div class="allName">{{v.name}}</div>
						<div class="allContent">{{v.content}}</div>
					</div>
				</div>
				<div class="mapSelect flexAlignCenter">
					<div class="mapSelects flexcenter" v-for="(v,i) in selectState"
						:class="{ 'mapSelects': v.state, 'nomapSelects': !v.state }" :style="{width:i==1?'27%':'24%'}"
						@click="mapselectClick(v)">{{v.name}}</div>
				</div>
				<div class="echartsmap">
					<lines :heightData="heightData" :options="lineOptions" v-if="trappedState==1"></lines>
					<TrappedLine :heightData="heightData" :options="lineOptions" v-if="trappedState==2">
					</TrappedLine>
					<pie :heightData="heightData" :options="chartOptions" v-if="trappedState==3"></pie>
					<malfunction :heightData="heightData" :options="malfunctionOptions" v-if="trappedState==4">
					</malfunction>
				</div>
			</div>
			<div class="trappedplace" v-else  @mouseleave="placeMouse" @mouseenter="placeMouseEnter">
				<div class="mapSelect flexAlignCenter">
					<div class="mapSelects flexcenter" v-for="(v,i) in placeselectState"
						:class="{ 'mapSelects': v.state, 'nomapSelects': !v.state }"
						:style="{width:100/placeselectState.length+'%'}" @click="placeSelectClick(v)">{{v.name}}</div>
				</div>
				<div class="echartsmap">
					<placeLine :heightData="heightData" :options="lineOptions" v-if="placeState==1">
					</placeLine>
					<placePie :heightData="heightData" :options="chartOptions" v-if="placeState==2"></placePie>
					<placebars :heightData="heightData" :options="placebarsOptions" v-if="placeState==3"></placebars>
				</div>
			</div>
		</div>
		<div class="select">
...
<script>
	import Pie from '@/echarts/trapped/pie';
	import lines from '@/echarts/trapped/lines';
	import TrappedLine from '@/echarts/trapped/TrappedLine';
	import malfunction from '@/echarts/trapped/malfunction';
	import placebars from "@/echarts/trapped/place/placebars"
	import placeLine from "@/echarts/trapped/place/placeLine"
	import placePie from "@/echarts/trapped/place/placePie"
	import {
	...
	export default {
		...
		methods: {
			...
			// 场所一揽
			placeUpdate() {
				this.placeselectState.forEach((item, index) => {
			...
			placeTimer() {
				...
				this.placeIntervalId = setInterval(() => {
					this.placeIndex = (this.placeIndex + 1) % this.placeselectState.length;
					this.placeUpdate();
				}, 5000); // 每5秒更新一次
			},
			placeMouse(){
			...
			placeSelectClick(v){
				this.placeselectState.forEach((res, index) => {
					if (v.name === res.name) {
						res.state = true;
						this.placeState = index + 1;
	...
	.content {
		...
		.select {
			...
			.selects {
				width: 100%;
				height: 50%;
				background-image: url(@/assets/img/newlargeScreen/norightSelect.png);
				background-size: 100% 100%;
				font-weight: bold;
				font-size: 0.78125vw;
				color: #FFFFFF;
			...
			.noselects {
				width: 100%;
				height: 50%;
				background-image: url(@/assets/img/newlargeScreen/rightSelect.png);
				background-size: 100% 100%;
				font-weight: bold;
				font-size: 0.78125vw;
				color: #FFFFFF;
