	...
	<div class="content flex">
		...
		<div class="contents">
			<div class="title">
				<div class="titles justifyBetween">
					<div class="titleLeft flexAlignCenter">
						<div v-for="(v,i) in maintenanceState" :class="{ 'mytitle': v.state, 'nomytitle': !v.state }"  class="flexcenter" @click="maintenanceClick(v)">
							{{v.name}}
						</div>
					</div>
					<div class="TitleRight" @click="maintenance()">
						详细统计 >
					</div>
				</div>
			</div>
			<div class="trappedContents" v-if="trappedContentsState"  @mouseleave="handleMouseLeave" @mouseenter="handleMouseEnter">
				...
				<div class="mapSelect flexAlignCenter">
					<div class="mapSelects flexcenter" v-for="(v,i) in behaviorState"
						:class="{ 'mapSelects': v.state, 'nomapSelects': !v.state }"   style="width:33%"
						@click="mapselectClick(v)">{{v.name}}</div>
				</div>
				<div class="echartsmap">
					<maintenancelines :heightData="heightData" :options="lineOptions" v-if="behaviorStates==1">
					</maintenancelines>
					<maintenanceslines :heightData="heightData" :options="lineOptions" v-if="behaviorStates==2">
					</maintenanceslines>
					<maintenancepie :heightData="heightData" :options="chartOptions" v-if="behaviorStates==3">
					</maintenancepie>
					<maintenance :heightData="heightData" :options="malfunctionOptions" v-if="behaviorStates==4">
					</maintenance>
				</div>
			</div>
			<div class="trappedplace" v-else  @mouseleave="placeMouse" @mouseenter="placeMouseEnter">
				...
				<div class="echartsmap">
					<maintenancePlaceLine :heightData="heightData" :options="lineOptions" v-if="placeState==1">
					</maintenancePlaceLine>
					<maintenancePlacePie :heightData="heightData" :options="chartOptions" v-if="placeState==2"></maintenancePlacePie>
					<maintenancePlacebars :heightData="heightData" :options="placebarsOptions" v-if="placeState==3"></maintenancePlacebars>
				</div>
			</div>
		</div>
	</div>
...
<script>
	import maintenancelines from '@/echarts/maintenance/maintenancelines';
	import maintenanceslines from '@/echarts/maintenance/maintenanceslines';
	import maintenancepie from '@/echarts/maintenance/maintenancepie';
	import maintenance from '@/echarts/maintenance/maintenance';
	
	
	import maintenancePlacebars from "@/echarts/maintenance/place/maintenancePlacebars"
	import maintenancePlaceLine from "@/echarts/maintenance/place/maintenancePlaceLine"
	import maintenancePlacePie from "@/echarts/maintenance/place/maintenancePlacePie"
	import {
	...
	export default {
		...
		methods: {
			...
			maintenance(){
				this.$store.commit('inspectionDrawers', true);
			},
			maintenanceClick(v){
				this.maintenanceState.forEach((res,index)=>{
					if(res.name==v.name){
						res.state=true
					}else{
						res.state=false
