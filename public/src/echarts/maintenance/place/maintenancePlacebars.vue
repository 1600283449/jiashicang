<template>
	<div :style="{ width: '100%', height: heightData }" class="justifyBetween">
		...
		<div id="maintenancePlacebars" :style="{ width: '70%', height: '100%' }"></div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		watch: {
			options: {
				handler(newVal, oldVal) {
					this.maintenancePlacebarsChart();
				},
				deep: true,
				immediate: true
			}
		...
		methods: {
			...
			maintenancePlacebarsChart() {
				try {
					if (!document.getElementById('maintenancePlacebars')) {
						return;
					}
					...
					this.malfunctionEchart = echarts.init(document.getElementById('maintenancePlacebars'), null, {
