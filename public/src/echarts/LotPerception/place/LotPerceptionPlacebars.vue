<template>
	<div :style="{ width: '100%', height: heightData }" class="justifyBetween">
		...
		<div id="LotPerceptionPlacebars" :style="{ width: '70%', height: '100%' }"></div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		watch: {
			options: {
				handler(newVal, oldVal) {
					this.LotPerceptionPlacebarsChart();
				},
				deep: true,
				immediate: true
			}
		...
		methods: {
			...
			LotPerceptionPlacebarsChart() {
				try {
					if (!document.getElementById('LotPerceptionPlacebars')) {
						return;
					}
					...
					this.malfunctionEchart = echarts.init(document.getElementById('LotPerceptionPlacebars'), null, {
